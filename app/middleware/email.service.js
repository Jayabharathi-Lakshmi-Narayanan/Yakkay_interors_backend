 ...
 */
 const sendResetPasswordEmail = async (to, token) => {
  ...
  const resetPasswordUrlwithbase =  `${baseappurl}/auth/reset-password?token=${token}`;
...
const createorderemail = async (to, workOrderNumber) => {
  const subject = 'Order created';
  const text = `Dear user,
    Thank you for placing order . Order will be approved within 4-6 working days and you can view the status of the order in your order list.
    Order Id: ${workOrderNumber}
...
const sendConfirmBookingMail = async (to, firstName, lastName, professional, profbooking, existingSlot, professionalData) => {
  ...
  const storeregister = async (to,storeName ) => {
    const subject = 'Store created';
    const text = `Dear ${storeName},
      Thank you for creating a  Store . 
